...
import { gsapAtom, champSelectAnimationsPlayedAtom } from "@/atoms/gsapAtom";
...
export function useChampSelectAnimation(skipAnimationsAndQueue: boolean) {
  ...
  const [champSelectAnimationsPlayed, setChampSelectAnimationsPlayed] = useAtom(
    champSelectAnimationsPlayedAtom
  );

  useGSAP(() => {
    if (champSelectAnimationsPlayed) {
    ...
    if (skipAnimationsAndQueue) {
      ...
      setAnimationComplete(true);
      setChampSelectAnimationsPlayed(true);
      return;
    }
  ...
  }, [skipAnimationsAndQueue, setAnimationComplete, setChampSelectAnimationsPlayed, champSelectAnimationsPlayed]);
}
