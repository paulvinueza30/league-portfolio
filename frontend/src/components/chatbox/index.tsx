...
import { chatAtom, addChatMessageAtom, initialChatScript } from "@/atoms/chatAtom";
...
function Chat() {
  ...
  const audioRef = useRef(new Audio(chatSound));
  ...
  useEffect(() => {
    if (chat.length === 0 || skipAnimationsAndQueue) return;
    const latestMessage = chat[chat.length - 1];

    if (latestMessage && latestMessage.type === "user") {
      try {
        const chatFx = audioRef.current;
  ...
  }, [chat, volume, skipAnimationsAndQueue]);
...
export default function Chatbox() {
  ...
  useEffect(() => {
    if (chatInitialized.current || chat.length > 0) return;

    chatInitialized.current = true;

    if (skipAnimationsAndQueue) {
      initialChatScript.forEach((msg) => {
        addChatMessage(msg);
      });
      return;
    }
  ...
  }, [addChatMessage, chat.length, skipAnimationsAndQueue]);
