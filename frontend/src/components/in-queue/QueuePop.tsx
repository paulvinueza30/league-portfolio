...
import { skipAnimationsAndQueueAtom, battleReadyAtom } from "@/atoms/queueAtom"; // Import battleReadyAtom
...
export default function QueuePop() {
  ...
  const [battleReady] = useAtom(battleReadyAtom); // Read battleReadyAtom
  const [locallyAccepted, setLocalAccepted] = useState<boolean>(false);
  const { volume } = useAudio();

  useEffect(() => {
    if (skipAnimationsAndQueue) return; // Don't play sound when skipping animations
    if (!battleReady) return; // Only play match audio if battle is ready
    matchAudio.currentTime = 0;
    matchAudio.volume = volume;
    matchAudio.play().catch((e) => console.warn("Playback error:", e));
  }, [skipAnimationsAndQueue, battleReady, volume]); // Added volume to dependencies
  ...
  useEffect(() => {
    ...
    if (skipAnimationsAndQueue || !battleReady) return; // Only run animations if battle is ready and not skipping
  ...
  }, [skipAnimationsAndQueue, locallyAccepted, battleReady]); // Added battleReady to dependencies

  // Don't render queue pop if skipping animations
...
function AcceptButton({
  ...
  function handleClick() {
    ...
    timeoutRef.current = setTimeout(() => {
      navigate("/champ-select"); // Navigate to /champ-select after timeout
    }, 1600);
  }
  ...
  return (
    <div className="flex justify-center select-none">
      <svg
        viewBox="-2 0 5.5 1.5"
        className={`transition-transform duration-150 w-[12em] h-[2.2em] sm:w-[16em] sm:h-[3em] md:w-[18em] md:h-[3.2em] lg:w-[20em] lg:h-[3.5em] ${!locallyAccepted && "hover:scale-105 active:scale-95"
          }`}
        style={{
...
function DeclineButton() {
  const lastHoverTimeRef = useRef(0);
  const { volume } = useAudio(); // Access volume from AudioContext
  ...
  function handleHover() {
    ...
    lastHoverTimeRef.current = now;
    hoveredSound.currentTime = 0;
    hoveredSound.volume = volume; // Use current volume for hover sound
    hoveredSound.play();
  }

  function handleClick() {
    navigate("/decline"); // Redirect to the new Decline page
  }
  ...
  return (
    ...
    hover:scale-105 active:scale-95 transition-transform" /* Added animation classes */
      onMouseEnter={handleHover}
