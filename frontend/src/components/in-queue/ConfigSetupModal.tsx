...
import { skipAnimationsAndQueueAtom, battleReadyAtom } from "@/atoms/queueAtom";
import { useAtom, useSetAtom } from "jotai";
import { useEffect } from "react";

import { Checkbox } from "@/components/ui/Checkbox";

export function ConfigSetupModal() {
  const [configured, setConfigured] = useState(false);
  const [skipAnimationsAndQueue, setSkipAnimationsAndQueue] = useAtom(skipAnimationsAndQueueAtom);
  const [localSkipAnimations, setLocalSkipAnimations] = useState(skipAnimationsAndQueue);
  const { volume, setVolume } = useAudio();
  const sliderValue = [Math.round(volume * 100)];
  const setBattleReady = useSetAtom(battleReadyAtom);
  const navigate = useNavigate(); // Initialize useNavigate

  // Reset states when modal opens (when returning to config)
  useEffect(() => {
    setBattleReady(false);
    setSkipAnimationsAndQueue(false);
  }, [setBattleReady, setSkipAnimationsAndQueue]);

  useEffect(() => {
    setLocalSkipAnimations(skipAnimationsAndQueue);
          ...
          <div className="w-full space-y-3 sm:space-y-4 bg-[#1A1F24] p-4 sm:p-6 rounded border border-[#2C2F34]">
            <div className="flex justify-between items-center gap-2">
             ...
             <div className="space-y-2">
               <label className="flex items-center gap-2 cursor-pointer">
                 <Checkbox
                   checked={localSkipAnimations}
                   onChange={(e) => setLocalSkipAnimations(e.target.checked)}
                 />
                 <span className="text-xs sm:text-sm font-medium text-[#A09B8C] uppercase tracking-wide">
            ...
            <Button
              onClick={() => {
                 if (localSkipAnimations) {
                   setSkipAnimationsAndQueue(true);
                   setBattleReady(true);
