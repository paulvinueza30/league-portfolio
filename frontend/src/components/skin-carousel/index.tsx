...
export default function SkinCarousel() {
    ...
    return (
      <>
        {skinArr.map((skin: Skin, idx) => (
          <CarouselItem
            key={skin.skinName}
            className={`cursor-pointer basis-1/5 flex justify-center ${skin.skinImg === defaultSkin.skinName && "joy-skin-showcase"
              }`}
            onClick={() => {
              api?.scrollTo(idx);
              setSkin(skinArr[idx]);
  ...
  return (
    <div className="w-full max-w-[12rem] sm:max-w-[16rem] md:max-w-[20rem] lg:max-w-[24rem] xl:max-w-[28rem] 2xl:max-w-[39.5em] h-full flex flex-col gap-2 sm:gap-3 md:gap-4 lg:gap-5 items-center justify-self-end text-white mt-82">
